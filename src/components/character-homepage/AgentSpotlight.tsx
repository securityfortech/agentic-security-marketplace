...
const AgentSpotlight = ({ agents, onHireAgent }: AgentSpotlightProps) => {
  return (
    <section className="mb-12">
      <h2 className="text-2xl font-bold mb-6">Featured Security Agents</h2>
      
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {agents.map((agent) => (
          <div 
            key={agent.id} 
            className="relative overflow-hidden bg-card rounded-xl border border-border hover:border-primary/30 transition-all duration-300 flex flex-col h-full"
          >
            {agent.featured && (
